...
Start
    ;set up some ports
    MOVLW B'00000000'	;All PORTF pins configured as an output
    MOVWF TRISF

    ; Display number 4 on U1
    MOVLW B'11111110' ; send PIN RH0 of PORT H low to turn on the transistor Q1
    MOVWF LATH ; via LATH to drive transistor Q1
    MOVLW B'11111110' ; set TRISH such that pin RH0 of PORTH is an output
    MOVWF TRISH
    MOVLW B'00110101' ; send f,g,b,c low
    MOVWF LATF ; via LATF to display number 5 on U1
...
outerdelay ;wait 20ms = 50 000 (50 000*0.4us = 20ms)
    ...
    ; Display number 6 on U2
    MOVLW B'11111101' ; send PIN RH1 of PORT H low to turn on the transistor Q2
    MOVWF LATH ; via LATH to drive transistor Q2
    MOVLW B'11111101' ; set TRISH such that pin RH1 of PORTH is an output
    MOVWF TRISH
    MOVLW B'00000110' ; send PINS a,f,g,e,d,c low
    MOVWF LATF ; via LATF to display number 2 on U2
    
    ;wait 20ms
    MOVLW 100
    MOVWF outerloop ;count=2
        
outer2delay ;wait 20ms = 50 000 (50 000*0.4us = 20ms)
    ;Wait 200us = 500 cycles (500*0.4us = 200us) 
    CALL dly200us     ;count = 502
    DECF outerloop
    BNZ outer2delay	;count=505
    ;end of outer loop
    ;count~50500
